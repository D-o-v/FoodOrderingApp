...
function MakeOrder() {
  ...
  // const downloadAsPDF = () => {
  //   // Create a new jsPDF instance with a wider page size
  //   // 'landscape' orientation makes the page wider
  //   // You can also adjust the dimensions (297, 210) to make it even wider if needed
  //   const pdf = new jsPDF({
  //     orientation: 'landscape',
  //     unit: 'mm',
  //     format: [297, 210] // A4 dimensions in landscape
  //   });
  
  //   let yOffset = 10;
  //   let xOffset = 10; // Added to control horizontal positioning
  
  //   pdf.setFontSize(18);
  //   pdf.text(`Orders for ${selectedDate}`, xOffset, yOffset);
  //   yOffset += 10;
  
  //   pdf.setFontSize(12);
  //   orders.forEach((order, index) => {
  //     if (yOffset > 190) { // Adjusted for landscape
  //       pdf.addPage();
  //       yOffset = 10;
  //     }
  
  //     pdf.setFontSize(14);
  //     pdf.text(`Order ${index + 1}: ${order.user.username}`, xOffset, yOffset);
  //     yOffset += 7;
  ...
  //         yOffset = 10;
  //       }
  //       const line = `${product.product.name} x ${product.quantity}: ₦${(product.product.price * product.quantity).toFixed(2)}`;
  //       pdf.text(line, xOffset + 5, yOffset);
  //       yOffset += 7;
  //     });
  
  //     pdf.setFontSize(14);
  //     pdf.text(`Total: ₦${order.total.toFixed(2)}`, xOffset + 5, yOffset);
  //     yOffset += 10;
  //   });
  
  //   pdf.save(`orders-${selectedDate}.pdf`);
  // };
  ...
  const downloadAsPDF = () => {
    ...
    const tableData = orders.flatMap(order => [
      ...
      [{ content: `Total: ₦${order.total.toFixed(2)}`, colSpan: 3, styles: { fontStyle: 'bold', halign: 'right' } }],
